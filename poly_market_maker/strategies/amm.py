...
class AMM:
    ...
    def phi(self):
        """Part du collateral a allouer a cette jambe.

        Correctif amont : quand le prix atteint une borne de la plage
        (`p_min`/`p_max`), la fenetre d'achat se vide et `p_i == p_l`, ce qui
        provoquait une division par zero ou un IndexError. Le cas se produit
        des qu'un marche derive vers sa resolution. On renvoie alors 0 :
        aucun achat n'est possible de ce cote, donc aucun collateral a lui
        allouer.
        """
        if not self.buy_prices:
            return 0.0

        denominator = sqrt(self.p_i) - sqrt(self.p_l)
        if denominator <= 0:
            return 0.0
